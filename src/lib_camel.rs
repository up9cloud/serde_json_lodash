/// Batch of [build_camel_link!] camelCase aliases. `build_camel_link!` is
/// defined in `macros.rs` so it is also usable from the category `mod.rs`
/// files (for camelCase aliases declared next to their targets).
...
// camelCase spellings of the snake_case API. Each is an alias (fn + macro, plus
// the `X` `_x` variants) of the canonical name, via `build_camel_link!`.
build_camel_links![
