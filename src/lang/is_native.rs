use crate::lib::Value;

/// See lodash [isNative](https://lodash.com/docs/#isNative)
///
/// There is no such type in JSON, so it always returns `false`
/// Additional cases:
///
/// ```rust
/// # use serde_json_lodash::is_native;
/// # use serde_json::json;
/// assert_eq!(is_native(&json!({})), false);
/// ```
pub fn is_native(_v: &Value) -> bool {
    false
}

/// Based on [is_native()]
///
/// Examples:
///
/// ```rust
/// #[macro_use] extern crate serde_json_lodash;
/// use serde_json::json;
/// // js version could be true for real `isNative` values, but those are not portable to JSON
/// assert_eq!(is_native!(&json!({})), false);
/// assert_eq!(is_native!(&json!("a")), false);
/// ```
///
/// Additional cases:
///
/// ```rust
/// # #[macro_use] extern crate serde_json_lodash;
/// # use serde_json::json;
/// assert_eq!(is_native!(), false);
/// assert_eq!(is_native!(&json!(null)), false);
/// ```
#[macro_export]
macro_rules! is_native {
    () => {
        false
    };
    ($a:expr $(,)*) => {
        $crate::is_native($a)
    };
    ($a:expr, $($rest:tt)*) => {
        $crate::is_native($a)
    };
}
